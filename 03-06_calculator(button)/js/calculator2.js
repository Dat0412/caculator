...
function keydownEvent(event) {
  ...
  // click enter or = to show result
  if (event.key === "Enter" || event.key === "=") {
    dspResult();
  }

  // click escape or c or delete to clear
  if (event.key === "Escape" || event.key.toLowerCase() === "c" || event.key === "Delete") {
    clear();
  }

  // click backspace
  if (event.key === "Backspace") {
    let current = elementResult.innerHTML.toString();
    if (current.length > 1) {
      elementResult.innerHTML = current.slice(0, -1);
    } else {
      elementResult.innerHTML = "0";
    }
  }
